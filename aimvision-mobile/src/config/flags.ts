 ...
 *
 * Default is `env.appEnv !== 'production'`: ON in dev + staging Release
 * builds so the team (and design partners) can capture real range footage
 * without configuring Statsig, and OFF in production builds so a shipped
 * customer build can never reach the dev capture screen. A production
 * Statsig gate can still flip it on for a specific internal build/user.
 * This is the in-code enforcement of the ADR-0009 constraint
