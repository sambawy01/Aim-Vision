...
2. **No explicit Article 9 consent mechanism (GDPR Art. 9(2)(a))** — Pose keypoints are biometric data; HealthKit/Whoop/HR data is health data. Standard consent fails — needs *explicit*, granular, separable, withdrawable consent per data category. Sprint 1 "athlete consent flow drafted" is far too generic.
3. **Egypt PDPL licensing not started (Law 151/2020 Arts. 4, 12, 19)** — Processing biometric/health data in Egypt requires Personal Data Protection Center (PDPC) license *before* collection. Cross-border transfer to EU/US requires separate PDPC permit. Penalties: EGP 100K–5M and imprisonment for officers.
