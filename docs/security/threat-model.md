...
- **React Native mobile app (iOS + Android)** — pairs with the GoPro, uploads video to the backend, drives the athlete UX, surfaces coaching reports, and is the *only* trusted holder of athlete identity at capture time.
...
The combination of *compromised club dashboard* and *abusive ex-coach* is the most under-appreciated risk: both are insiders with legitimate scopes, and most product harms come from over-broad scope rather than from external break-ins.
...
| I      | Compromised club dashboard tries to read Solo history with the redemption response.                                  | Redemption response is an *attribution-write-only* ephemeral capability scoped to one `session_id`. It cannot read history. Endpoint /users/{id}/sessions rejects this capability. |
...
| I      | Pretrained-model leakage of base-corpus PII.                                                                                     | Documented limitation: self-hosted Ollama prevents *new* egress but cannot unlearn the pretraining corpus. Pinned model selected for low-leakage profile; outputs filtered for PII patterns.       |
...
| R      | Cloud admin makes a change in federation tenant and denies it.                                                    | Break-glass workflow: admin must request, justify, get approval; action is audit-logged into the *federation's own* SIEM, not back to cloud.                                                 |
...
- **Blast radius:** all cloud-tier data (Solo + Club) within the limits of admin-role permissions; KMS calls *only those keys the role can use*; audit log writeable but not reissue-able (hash chain is tamper-evident).
...
- **Detection:** federation-owned SIEM (cloud is *not* the audit chain of custody for federations); audit chain hash break.
...
- **R20. QR token cryptographic design fails review** — Probability: Low / Impact: Critical. Mitigation: design ratified per `docs/security/qr-checkin-token-spec.md` *and* second-pair review by external cryptographer before Sprint 16 implementation.
