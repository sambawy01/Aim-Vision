...
- **ChArUco 12×9** with 30 mm squares and 22 mm AR markers (DICT_4X4_100). The federation-kit printed plate uses DICT_4X4_100 because a 12×9 chessboard has 54 white cells and DICT_4X4_50's 50-marker library can't cover them all; the 100-marker variant is the smallest dictionary that fits the spec board geometry.
...
- `aimvision_ml.inference.camera_calibration` (Python, in `aimvision-ml/`) ships the math layer of step 3 above as a pure numpy + scipy bundle-adjustment solver. Public surface: `ChArUcoBoard` (3D corner geometry for the 12×9 spec board), `CameraIntrinsics` + `CameraExtrinsics` (matching the persistence schema in §4.5), `project_points` (pinhole + Brown-Conrady 5-parameter distortion, OpenCV-compatible coefficient layout), and `refine_calibration` (joint refinement via `scipy.optimize.least_squares` with Rodrigues 3-vector rotation parameterization). 12 synthetic-board tests cover projection sanity, distortion behavior, noise-free intrinsic recovery, noise-tolerant focal recovery (within 3% at 0.3 px corner noise), radial-distortion recovery, and input validation.
- `aimvision_ml.inference.charuco_detect` (in the optional `aimvision-ml[vision]` extra, gated on `opencv-python-headless`) ships the detection + initial-seed stages: `detect_charuco_corners(image, board)` wraps the modern OpenCV `CharucoDetector` API for sub-pixel-refined corner extraction; `seed_calibration_from_detections` uses `board.matchImagePoints` + `cv2.calibrateCamera` (the post-OpenCV-4.7 replacement for the removed `calibrateCameraCharuco` helper) to produce the initial intrinsics + per-view extrinsics that feed the bundle-adjustment refinement above. `render_charuco_board_image` produces a printable board image for the federation kit. 7 cv2-gated tests cover render → detect round-trip, blank-image graceful degradation, input validation, end-to-end seed → refine pipeline. CI runs the base ML container without the `vision` extra so the wheel doesn't bloat the default image; the cv2-gated tests skip silently in that path and run when the dev locally installs `uv sync --extra vision`.
