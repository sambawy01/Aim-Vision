...
3. **Slice 3:** Real-time frame processor — three independently-shippable sub-slices because the surface area touches RN, native Swift/Kotlin, AND Rust. Each sub-slice is verifiable on its own:
   ...
   - **Slice 3b (next):** Native frame processor plugin — Swift + Kotlin classes installed via an Expo config plugin local to the repo. The plugin receives the raw CMSampleBuffer / ImageProxy and exposes a callable that the JS worklet invokes; today it just logs frame metadata, tomorrow it forwards to slice 3c.
   - **Slice 3c (after):** C ABI bridge — the native plugin from 3b calls into `aimvision-camera-phone` via the `extern "C"` media plane described in ADR-0003. End-to-end zero-copy frame data path: VC worklet → Swift/Kotlin → Rust `PhoneCamera::push_frame`. The phone backend becomes the _third_ `CameraMedia` implementation alongside the mock and the (still pending) GoPro backend.
